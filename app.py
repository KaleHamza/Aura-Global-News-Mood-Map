...
def verileri_yukle():
    try:
        ...
        st.error(f"Veritabanı okuma hatası: {e}") # Hatayı dashboard'da gör
        return pd.DataFrame()
...
if not config.GOOGLE_API_KEY:
    st.error(" GOOGLE_API_KEY .env dosyasında tanımlanmamış!")
else:
    try:
...
if not df.empty:
    ...
    with tabs[3]: # Kelime Bulutu
        ...
        metin = " ".join(df[df['ulke'] == ulke]['baslik'])
        if len(metin) > 10:
    ...
    with tabs[4]: # Versus
        ...
        sel2 = u2.selectbox("2. Ülke", sorted(df['ulke'].unique()), index=1)
        st.plotly_chart(px.line(df[df['ulke'].isin([sel1, sel2])], x='tarih', y='skor', color='ulke'), use_container_width=True)
    ...
    with tabs[5]: # ŞİRKET TAKİBİ
        ...
        if s_data:
            ...
            st.info(f"Bugün toplam {len(sdf)} dev teknoloji şirketi global gündemde yer alıyor.") # Bilgi mesajı
            ...
            st.warning("Henüz takip edilen şirketlerle ilgili bir haber düşmedi. Veritabanı güncellendiğinde burada görünecekler.")
    ...
    st.dataframe(df[df['ulke'] == secilen].sort_values(by='tarih', ascending=False)[['tarih', 'baslik', 'skor', 'kategori', 'url']], use_container_width=True)

else:
    st.warning("Veri bekleniyor... Lütfen main.py'yi çalıştırın.")